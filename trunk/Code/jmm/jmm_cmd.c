...
static int jmm_newsock_handler_wf(cJSON* js)
{
    char* data = cJSON_PrintUnformatted(js);
    printf("new sock handler: %s\n", data);
    free(data);

    cJSON* js_arg = cJSON_GetObjectItem(js, JMM_CMD_REQ_ARG);
    cJSON* sock = cJSON_GetObjectItem(js_arg, JMM_CMD_NEWSOCK_SOCK);
    int sf = sock->valueint;
    ...
    for (i = 0; i < shm_wf->proc_svr_num; i++) {
        ...
        if (shm_sock->sock_fd == 0) {
            //set up event;
            printf("pid=%d, init new sock, idx=%d, sock:%d\n", shm_wf->pid, i, sf);
            int newsf=-1;
            jmm_recv_fd(shm_wf->mother_fd, &newsf);
            printf("receive new:%d, event:%d, mother:%d\n", newsf, (int)shm_wf, shm_wf->mother_fd);
